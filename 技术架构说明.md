...
```properties
# application.properties
spring.datasource.url=jdbc:mysql://localhost:3306/hospital?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=Asia/Shanghai&characterEncoding=utf8mb4
spring.datasource.username=root
spring.datasource.password=你的MySQL密码  # ⚠️ 需要修改为自己的密码
...
## 5. 数据库设计（MySQL 8.0）
### 5.1 主要表（节选，字段可按实现细化）
- `patients(id, account, password_hash, real_name, phone, age, height, weight, avatar_url[TEXT], created_at)`
- `doctors(id, account, password_hash, name, status, avatar_url[TEXT], created_at, deleted_at)`
...
- `appointments(id, patient_id, doctor_id, department_id, doctor_name, department_name, visit_at, status, illness_desc, created_at)`
- `visits(id, patient_id, doctor_id, department_id, visit_at, appointment_id, total_fee, doctor_advice, status, created_at)`
- `items(id, name, price, unit, type)`
- `visit_items(id, visit_id, item_id, item_name, unit_price, quantity, total_amount)`
- `reviews(id, patient_id, doctor_id, department_id, target, visit_id, score, comment, created_at)`
- `reminders(id, appointment_id, trigger_at, channel, status, sent_at)`
...
### 9.3 数据库
- MySQL 8.0，需要修改 `application.properties` 中的密码为自己的 MySQL 密码
- 使用 `init_hospital_db.sql` 初始化数据库（包含初始科室：儿科/口腔科/内科/外科）
